...
function computeUtilization(total, max) {
    if (max <= 0)
        return 0;
    return Number(((total / max) * 100).toFixed(2));
}
function buildSubsetList(orders) {
...
function bestForOrders(truck, orders) {
    // Meet-in-the-middle:
    ...
    for (const a of L) {
        if (a.weight > truck.max_weight_lbs || a.volume > truck.max_volume_cuft)
            continue;
        for (const b of R) {
...
function selectOrdersByMask(orders, mask) {
    const ids = [];
...
function optimize(truck, orders) {
    ...
    let globalBest = { selected_order_ids: [], payout: 0n, weight: 0, volume: 0 };
