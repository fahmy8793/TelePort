import { OptimizeRequestBody, Order } from "../types/models";
...
function validateOrder(o: any, idx: number): string[] {
  const errors: string[] = [];

  if (!isNonEmptyString(o?.id)) errors.push(`orders[${idx}].id is required`);
  ...
  if (!isNonEmptyString(o?.origin)) errors.push(`orders[${idx}].origin is required`);
  if (!isNonEmptyString(o?.destination)) errors.push(`orders[${idx}].destination is required`);

  if (!isNonEmptyString(o?.pickup_date)) errors.push(`orders[${idx}].pickup_date is required`);
  if (!isNonEmptyString(o?.delivery_date)) errors.push(`orders[${idx}].delivery_date is required`);
  ...
  if (!Number.isFinite(p)) errors.push(`orders[${idx}].pickup_date must be YYYY-MM-DD`);
  if (!Number.isFinite(d)) errors.push(`orders[${idx}].delivery_date must be YYYY-MM-DD`);
...
export function validateOptimizeRequest(body: any): { ok: true; value: OptimizeRequestBody } | { ok: false; errors: string[] } {
  const errors: string[] = [];
  ...
  if (!truck || typeof truck !== "object") {
    ...
    if (!isNonEmptyString(truck.id)) errors.push("truck.id is required");
  ...
  if (!Array.isArray(orders)) {
    ...
    if (orders.length > 22) errors.push("orders length must be <= 22");
  ...
  return { ok: true, value: { truck, orders: orders as Order[] } };
}
